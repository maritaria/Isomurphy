...
def readgraphlist(readline):
	...
	contin = True
	while contin:
		G, newoptions, contin = readgraph(readline)
		options += newoptions
		L.append(G)
	return L, options


def loadgraph(filename: str, loadList: bool) -> Graph:
	"""
...
def loadgraphs(filename: str) -> list:
	"""
	Reads the file <filename>, and returns the corresponding list of Graph objects.
	The output is a 2-tuple, where the first item is a list of graphs,
	and the second is a list of options (strings).
	"""
	readfile = open(filename, 'rt')
...
def inputgraph(readlist : bool=False) -> Graph:
	"""
...
def writegraphlist(graphList: list, writeline, options=[]):
	"""
	For internal use.
	"""
...
def printgraph(GL, options=[]):
	...
	Optional last argument: as list of options that will be included in the header.
...
def writeDOT(G: Graph, filename: str, directed: bool = False):
	"""
